...
public class List1_Brisenje_povekekratni_jazli {
	...
	public static void main(String[] args)throws IOException {
		...
		int br = 0;
		while(node.succ != null) {           // Mora da e node.succ zatoa so u if sporeduvam so node.succ;
			if(node.element == node.succ.element) {
				lista.delete(node.succ);
				br++;						// AKO E ZA BRISENJE IZBRISI I NE GO ZGOLEMUVAJ 
			}
			else {							// !!!!!!!!!!! ZAD moce da se rese i na nacin so mesto da go briseme node.succ briseme go NODE
				node = node.succ;
			}
		}
